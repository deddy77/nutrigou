...
export const Route = createFileRoute("/_authenticated/calcul")({
  head: () => ({
    meta: [
      { title: "Calcul nutritionnel — NutriPlan" },
      {
        name: "description",
        content:
          "Calculez vos besoins caloriques quotidiens, la répartition en macronutriments et consultez l'historique de vos calculs.",
      },
      { property: "og:title", content: "Calcul nutritionnel — NutriPlan" },
      {
        property: "og:description",
        content:
          "Calculez vos besoins caloriques quotidiens et consultez l'historique de vos calculs.",
      },
...
function NutriPlan() {
  ...
  const historique = useQuery({
    queryKey: ["historique"],
    queryFn: async (): Promise<LigneHistorique[]> => {
      const { data, error } = await supabase
        .from("calculs")
        .select(
          "id, created_at, sexe, age, taille, poids, activite, objectif, calories, proteines, glucides, lipides",
        )
        .order("created_at", { ascending: false })
        .limit(20);
      if (error) throw error;
      return (data ?? []) as LigneHistorique[];
    },
  });
  ...
  const enregistrer = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("calculs").insert({
        user_id: userId,
        sexe,
        age,
        taille,
  ...
  const labelActivite = (v: string) =>
    ACTIVITE_LABELS.find((a) => a.value === v)?.label ?? v;
  const labelObjectif = (v: string) =>
    OBJECTIF_LABELS.find((o) => o.value === v)?.label ?? v;
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-6 flex justify-end">
          <button
            ...
            className="rounded-2xl border border-border bg-card/60 px-4 py-2 text-sm font-semibold text-muted-foreground transition-all hover:border-primary/50 hover:text-foreground"
          >
            Se déconnecter
          </button>
        </div>
        ...
        <section className="mb-10 rounded-3xl border border-border bg-card/70 p-6 backdrop-blur-sm soft-shadow sm:p-8">
          ...
          <button
            ...
            className="w-full rounded-2xl border border-primary/50 bg-primary/20 px-6 py-4 text-base font-semibold text-primary transition-all hover:bg-primary/30 disabled:opacity-60 soft-shadow"
          >
            {enregistrer.isPending ? "Enregistrement…" : "Calculer et enregistrer"}
          </button>
          {enregistrer.isError && (
          ...
          {enregistrer.isSuccess && !enregistrer.isPending && (
            <p className="mt-3 text-sm text-primary">
              Résultat enregistré dans votre historique.
            </p>
          )}
        </section>
        ...
        <section className="mb-10 rounded-3xl border border-border bg-card/70 p-6 backdrop-blur-sm soft-shadow sm:p-8">
          ...
          {historique.isLoading ? (
            ...
            <p className="text-sm text-muted-foreground">
              Aucun calcul enregistré pour le moment. Appuyez sur « Calculer et
              enregistrer » pour conserver un résultat.
            </p>
          ) : (
              ...
              {historique.data!.map((c) => (
                ...
                >
                  ...
                  <p className="mt-1.5 text-xs text-muted-foreground">
                    {c.sexe === "homme" ? "Homme" : "Femme"} · {c.age} ans ·{" "}
                    {c.taille} cm · {c.poids} kg · {labelActivite(c.activite)} ·{" "}
                    {labelObjectif(c.objectif)}
                  </p>
          ...
          <p className="mt-4 text-xs text-muted-foreground">
            Vos vingt derniers résultats enregistrés.
          </p>
        </section>
